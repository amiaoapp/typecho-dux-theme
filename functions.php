...
function themeConfig($form) {
    ...
    $tuijian = new Typecho_Widget_Helper_Form_Element_Text('tj_cid', NULL, NULL, _t('特殊提醒'), _t('请输入要置顶展示文章的cid'));
    ...
    $socialtwitter = new Typecho_Widget_Helper_Form_Element_Text('socialtwitter', NULL, NULL, _t('输入BILIBILI链接'), _t('在这里输入BILIBILI链接,支持 http:// 或 https:// 或 //'));
...
'before' => '',
'after' => '',
'xformat' => '<article class="excerpt-minic excerpt-minic-index"><h2><span class="red">【特殊提醒】</span><a href="{permalink}" title="{title}">{title}</a></h2><p class="note">{content}...</p></article>'
);
$db = Typecho_Db::get();
...
function slout() {
    ...
    if (!empty($options->slidercode)) {
		...
		$text='<a target="_blank" href="https://www.appmews.com/usr/themes/image"><img src="https://www.appmews.com/usr/themes/image/heihei.jpg"></a>
<a target="_blank" href="https://www.appmews.com/usr/themes/image"><img src="https://www.appmews.com/usr/themes/image/lol.jpg"></a>';
	}
	$t_arr = explode('
', $text);
...
function sitebar_ad($obj) {
    ...
    if (!empty($options)) {
		...
		$text="https://jq.qq.com/?_wv=1027&k=5s1mqeY\n点击链接加入群聊【月光下】：\n分享资源，交流经验...";
	}
	$b_arr = explode("\n", $text);
...
//获取评论最多的文章
function getHotComments($limit = 6){
    $db = Typecho_Db::get();
